      ...
      <div className="flex justify-between items-center max-w-7xl mx-auto">
        ...
        <div className="flex justify-between w-full md:w-auto md:gap-2">
          ...
          <div className="hidden md:flex gap-4">
            <Link to="/">
              <Button variant={location.pathname === '/' ? "default" : "ghost"} className="font-medium">Home</Button>
            </Link>
            <Link to="/location">
              <Button variant={location.pathname === '/location' ? "default" : "ghost"} className="font-medium">Find Mechanics</Button>
            </Link>
            {isLoggedIn ? (
              <Link to="/profile">
                <Button variant={location.pathname === '/profile' ? "default" : "ghost"} className="font-medium">Profile</Button>
              </Link>
            ) : (
              <Link to="/login">
                <Button variant={location.pathname === '/login' ? "default" : "ghost"} className="font-medium">Login</Button>
              </Link>
            )}
          </div>
        </div>
